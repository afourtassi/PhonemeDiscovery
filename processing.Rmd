...
#Here I stich new full data analysis (50k) with old small data (<50k). The updated small data analysis is runing, but there should be minor differences, if any!

data_small <- data_size %>%
  mutate(Segmentation = 'Ideal') %>%
  dplyr::rename(language = lang,
         variation =var,
         context = cont,
         dimension = dim,
         StD=`SD\t`) %>%
...
data_full <- data_all %>%
  mutate(size = '50k') %>%
  filter(Segmentation  =='Ideal' )

data_size_new <- data_full %>%
  bind_rows(data_small)
                            
data_size_new$size <- factor(data_size_new$size, levels = c('50k', '5k', '500', '50'))


ggplot(data_size_new, 
      aes(x = variation, y = AUC)) +
  geom_point(col='black', fill = "white", alpha = 0.05)+
  geom_point(data = subset(data_size_new, variation==25 | variation == 41), col='red', alpha = 0.05)+
