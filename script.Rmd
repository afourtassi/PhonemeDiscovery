...
Here show the main summary results 
...
ggplot(data_sum,  
       #aes(x = variation, y = mean, shape=Segmentation)) +
       aes(x = variation, y = mean)) +
  geom_point()+
  geom_line()+
  geom_pointrange(aes(ymin = sd_low, ymax = sd_up), 
                  position = position_dodge(width = .03), size=0.5) +
  geom_point(data=subset(data_sum, variation == 25 | variation == 41), col='red')+
...
ggplot(histo, 
  ...
  xlab("Variation") +ylab("Percent optimal")


```


here I show the distribution for the parameter setting dim=20, context=20
...
Here show the effect of data size

```{r echo=FALSE}


data_size <- read_delim("res/results_size.txt", delim = " ")

data_size$size <- mapvalues(data_size$size, 
                                     from = c('100%','10%','1%','0.1%'),
                                     to = c('50k', '5k', '500', '50'))
                            
data_size$size <- factor(data_size$size, levels = c('50k', '5k', '500', '50'))

ggplot(data_size, 
      aes(x = var, y = AUC)) +
  geom_point(col='black', fill = "white", alpha = 0.05)+
  geom_point(data = subset(data_size, var==25 | var == 41), col='blue', alpha = 0.05)+
  stat_summary(fun.y="mean", geom="point", col='red', size=0.5)+
  scale_x_log10() +
theme(aspect.ratio = 0.7, legend.title = element_text(size=8)) +  
  theme_few()+
  facet_grid(size ~ lang) +
  theme(aspect.ratio = 0.7)+
  xlab("Inventory size") +ylab("Differentiation score (AUC)")
